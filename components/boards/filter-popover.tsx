...
/** Filter payload shape matching the API contract */
...
export function FilterPopover({ filters = {}, onApply }: FilterPopoverProps) {
  const { data: dimensionsData } = useQuery({
    queryKey: ["clickhouse-dimensions"],
    queryFn: async () => {
  ...
  const { data: singleDimRes, isFetching } = useQuery({
    queryKey: [
  ...
  const filteredValues: string[] = singleDimRes?.values?.length
  ...
  // When dimensions load (or open with invalid category), default to first dimension
  useEffect(() => {
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <button className="relative h-9 w-9 flex items-center justify-center rounded-lg border border-border-mid bg-hover text-text-secondary hover:bg-border-subtle transition-colors">
