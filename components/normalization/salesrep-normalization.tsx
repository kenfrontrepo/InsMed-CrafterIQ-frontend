...
import {
  fetchSalesReps,
  updateSalesRepNorm,
  bulkUpdateSalesRepNorm,
  type SalesRepFilters,
...
const COLUMNS = [
  ...
  { key: "SalesPersonName", label: "Original Name", sortable: true, width: "20%" },
  { key: "NormSalesPersonName", label: "Normalized Name", sortable: true, editable: true, width: "20%" },
  {
  ...
  {
    key: "NormSalesPersonConfidenceLabel", label: "Confidence", sortable: false, width: "18%",
    render: (value: unknown) => {
...
export function SalesRepNormalization() {
  const queryClient = useQueryClient();
  const { filters: { salesrep: f }, setFilter, resetFilter } = useNormalizationFiltersStore();
  ...
  const baseFilters: Omit<SalesRepFilters, "page"> = {
    limit: 150,
    search: debouncedSearch || null,
    sort_by: sortBy as SalesRepFilters["sort_by"],
  ...
  const { data, isLoading, isFetchingNextPage, hasNextPage, fetchNextPage } = useInfiniteQuery({
    queryKey: ["salesreps", baseFilters],
    queryFn: ({ pageParam = 1 }) => fetchSalesReps({ ...baseFilters, page: pageParam }),
  ...
  const handleUpdate = useCallback(async (_id: string, rawName: string, normName: string) => {
    const response = await updateSalesRepNorm(rawName, normName);
    queryClient.invalidateQueries({ queryKey: ["salesreps"] });
    return response;
  }, [queryClient]);

  const handleSort = useCallback((column: string, direction: "asc" | "desc" | null) => {
    if (direction === null) { setFilter("salesrep", { sortBy: null, sortOrder: "asc" }); }
    else { setFilter("salesrep", { sortBy: column, sortOrder: direction }); }
  }, [setFilter]);
  ...
  const handleBulkApply = useCallback(async () => {
    ...
    if (!trimmed) return;
    const selectedRows = allData.filter(row => selectedIds.has(`${String(row.SalesPersonID)}-${String(row.SalesPersonName)}`));
    if (selectedRows.length === 0) return;
    const items = selectedRows.map(row => ({ raw_name: String(row.SalesPersonName), norm_name: trimmed }));
    setIsBulkUpdating(true);
    try {
      const response = await bulkUpdateSalesRepNorm(items);
    ...
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center gap-2">
        <div className="relative w-72">
          ...
          <input type="text" placeholder="Search by ID or name..." value={searchQuery}
            onChange={(e) => setFilter("salesrep", { searchQuery: e.target.value })}
      ...
      <div className="flex-1 min-h-0">
        <NormalizationTable
          ...
          idKey="SalesPersonID" rawNameKey="SalesPersonName" normNameKey="NormSalesPersonName"
