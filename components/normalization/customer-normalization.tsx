...
const COLUMNS = [
  ...
  { key: "CustomerName", label: "Original Name", sortable: true, width: "22%" },
  { key: "NormCustomerName", label: "Normalized Name", sortable: true, editable: true, width: "22%" },
  {
  ...
  {
    key: "NormCustomerConfidenceLabel", label: "Confidence", sortable: false, width: "14%",
    render: (value: unknown) => {
...
export function CustomerNormalization() {
  const queryClient = useQueryClient();
  // Persisted across tab switches via Zustand
  const { filters: { customer: f }, setFilter, resetFilter } = useNormalizationFiltersStore();
  const { normFilter, confidenceFilter, sortBy, sortOrder, searchQuery, revenueMin, revenueMax, dateFrom, dateTo } = f;
  const [debouncedSearch, setDebouncedSearch] = useState(searchQuery);

  // Ephemeral local state (ok to reset on tab switch)
  ...
  const baseFilters: Omit<CustomerFilters, "page"> = {
    limit: 150,
  ...
  const { data, isLoading, isFetchingNextPage, hasNextPage, fetchNextPage } = useInfiniteQuery({
    queryKey: ["customers", baseFilters],
    queryFn: ({ pageParam = 1 }) => fetchCustomers({ ...baseFilters, page: pageParam }),
  ...
  const handleUpdate = useCallback(async (_id: string, rawName: string, normName: string) => {
    const response = await updateCustomerNorm(rawName, normName);
    queryClient.invalidateQueries({ queryKey: ["customers"] });
    return response;
  }, [queryClient]);

  const handleSort = useCallback((column: string, direction: "asc" | "desc" | null) => {
    if (direction === null) { setFilter("customer", { sortBy: null, sortOrder: "asc" }); }
    else { setFilter("customer", { sortBy: column, sortOrder: direction }); }
  }, [setFilter]);
  ...
  const handleBulkApply = useCallback(async () => {
    ...
    if (!trimmed) return;
    const selectedRows = allData.filter(row => selectedIds.has(`${String(row.CustomerID)}-${String(row.CustomerName)}`));
    if (selectedRows.length === 0) return;
    const items = selectedRows.map(row => ({ raw_name: String(row.CustomerName), norm_name: trimmed }));
    setIsBulkUpdating(true);
    try {
      const response = await bulkUpdateCustomerNorm(items);
    ...
    <div className="flex flex-col h-full gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative w-72">
          ...
          <input
            ...
            value={searchQuery}
            onChange={(e) => setFilter("customer", { searchQuery: e.target.value })}
        ...
        <Popover open={filtersOpen} onOpenChange={handleFiltersOpen}>
          <PopoverTrigger asChild>
            <button className="ml-auto flex items-center gap-2 h-8 px-3 border border-border-mid rounded-md text-sm text-text-secondary hover:bg-hover transition-colors whitespace-nowrap">
              ...
              <span>Filters</span>
              {activeFilterCount > 0 && (
                <span className="flex items-center justify-center h-4 min-w-[16px] px-1 rounded-full bg-slate-700 text-white text-[10px] font-medium leading-none">
                  {activeFilterCount}
                </span>
              )}
            </button>
          ...
          <PopoverContent align="end" className="w-[360px] p-0">
            ...
            <div className="px-4 py-4 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Slider
                  min={0} max={2000000000} step={1000000}
                  value={[
            ...
            <div className="px-4 py-3 border-t border-border-subtle flex gap-2">
              <button onClick={resetAllFilters}
                className="flex-1 px-3 py-2 text-sm border border-border-mid rounded-md text-text-secondary hover:bg-hover transition-colors">
                Reset
              </button>
              <button onClick={applyAllFilters}
                className="flex-1 px-3 py-2 text-sm font-medium bg-slate-900 text-white rounded-md hover:bg-slate-800 transition-colors">
                Apply filters
              </button>
            </div>
          </PopoverContent>
      ...
      <div className="flex-1 min-h-0">
        <NormalizationTable
          ...
          idKey="CustomerID" rawNameKey="CustomerName" normNameKey="NormCustomerName"
